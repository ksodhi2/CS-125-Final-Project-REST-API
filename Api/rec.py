...
def get_gened_for_attribute(sACP,sCS,sHUM,sNAT,sQR,sSBS):
    return courseDf[(courseDf["ACP"] == sACP ) & (courseDf["CS"] == sCS ) & (courseDf["HUM"] == sHUM ) & (courseDf["NAT"] == sNAT ) & (courseDf["QR"] == sQR ) & (courseDf["SBS"] == sSBS )][["Name","Number","GPA","Total Students"]].sort_values('GPA',ascending = False).drop_duplicates(subset ="Name", 
                     keep = 'first').head(5).to_json(orient='records')
